...
- ``keypress`` whose expected effect says open/launch/switch-to -> ``window_state``;
- everything else (click/double_click/drag/scroll/wait) -> ``visual_change``; when an
  expected effect is stated, a change is REQUIRED (unchanged screen = failed); with no
  stated expectation pixels alone stay ambiguous (identical screen -> ``uncertain`` ->
  recovery).
"""
...
from .validator import COORDINATE_ACTIONS, GroundingValidator, ValidationOutcome
...
class ComputerUseAgent:
    ...
    def _build_intent(
        ...
        if kind is None:
            ...
            elif action.action is ActionType.KEYPRESS and effect:
