...
COORDINATE_ACTIONS: frozenset[ActionType] = frozenset(
    {ActionType.CLICK, ActionType.DOUBLE_CLICK, ActionType.DRAG}
)
...
class ValidationOutcome(GroundingValidation):
    ...
    ``window_not_allowed``, ``point_out_of_bounds``, ``missing_point``,
    ``missing_text``, ``missing_keys``). ``error`` (private, never serialized) carries the
    first typed rejection instance for controllers that branch on exception type.
    """
...
class GroundingValidator:
    ...
    def validate(
        ...
        *,
        ...
        if action.action == ActionType.KEYPRESS and not action.keys:
            reject("missing_keys", "Keypress actions require at least one key.")
