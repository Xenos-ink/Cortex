...
F1–F7, plus the DRAG action and the compact-change verification upgrade). Every rule
below is enforced in a named module and exercised by the test suite (standard suite:
408 passed, 7 skipped — the skips are the gated real-Windows E2E desktop tests;
...
| `keyboard_shortcut_state_change` | keypress containing delete/backspace/win/alt/ctrl |
...
`low_routine_action` (keypress without state-changing keys, scroll, wait, done),
`completion` (done marker).

Policy merge rules (`evaluate`): the legacy gates run verbatim first (stopped session,
step budget, sensitive-typed-text block, state-changing keys, click/type approval
defaults); the classification then fills `risk`/`category`/`reason` and the approval
...
- **Checked before every physical input**: `backend.execute` calls
  ...
  hotkeys, and before scroll; a fired token means zero further inputs.
