...
the red-team fix round F1–F7), followed by the Cortex naming pass, the DRAG action, and
the compact-change verification upgrade. This document describes the code as it exists
...
suite (standard suite: **408 passed, 7 skipped** — the skips are the gated
...
| Perception/Grounding | `grounding.py` | `GroundingRouter` over `GroundingStrategy` implementations: coordinate (real), region descriptor (real), text-anchor (P1 stub), accessibility (P1 stub); fail-closed `UnsupportedGroundingError` |
| Action validation | `validator.py` | bounds, confidence floor, coordinate-space refusal, window/process allowlists, staleness + `source_observation_id` binding |
...
5. **VALIDATE** — a second fresh pre-execution observation is captured and
   ...
   Rejection → recovery (`codes` → FailureClass).
...
classifies drag like click/double-click. `risk` is filled only by the
...
1. **Tool surface**: the 6 tool names, stdio transport, and parameter positions are
   ...
   `computer_execute(..., expected_effect=None, x2=None, y2=None)` (the trailing
   `x2`/`y2` carry the drag end point). (`computer_observe` gained no new
...
delta=0, approved=False, expected_effect=None, x2=None, y2=None)`**
